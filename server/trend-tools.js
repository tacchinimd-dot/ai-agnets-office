...
해당 카테고리의 상품 수, 가격 통계, 주요 브랜드, 급상승 수, TOP 10 상품 목록`,
...

  {
    ...
    description: `네이버 검색 키워드 데이터를 KG API를 통해 조회합니다.

필터 가능 항목:
- filters_product: BRD_CD(브랜드코드, 필수), ITEM_GROUP(아이템그룹) 등 상품 필터
- filters_search_keyword: KWD_NM_CLASS(자사/경쟁사/일반/라이프스타일/모니터링키워드), KWD_BRD_NM(키워드 브랜드명) 등 키워드 분류 필터
- 기간: start_dt ~ end_dt (YYYY-MM-DD)

조회 가능 셀렉터(selectors):
- BRD_CD, KWD_NM, END_DT, KWD_NM_CLASS, KWD_BRD_NM 등

BRD_CD는 반드시 filters_product에 포함해야 합니다.`,
    input_schema: {
      type: 'object',
      properties: {
        filters_product: {
          ...
          items: {
            type: 'object',
            properties: {
              system_code: { type: 'string', description: '필터 값 (예: 브랜드코드)' },
              system_field_name: { type: 'string', description: '필터 필드명 (예: BRD_CD, ITEM_GROUP)' }
            },
            required: ['system_code', 'system_field_name']
          },
          description: '상품 필터 배열 (BRD_CD 필수 포함)'
        },
        filters_search_keyword: {
          type: 'array',
          items: {
            type: 'object',
            properties: {
              system_code: { type: 'string', description: '필터 값 (예: 자사, 경쟁사)' },
              system_field_name: { type: 'string', description: '필터 필드명 (예: KWD_NM_CLASS, KWD_BRD_NM)' }
            },
            required: ['system_code', 'system_field_name']
          },
          description: '검색 키워드 분류 필터 (KWD_NM_CLASS: 자사/경쟁사/일반/라이프스타일/모니터링키워드, KWD_BRD_NM)'
        },
        start_dt: {
          type: 'string',
        ...
        selectors: {
          ...
          items: {
            type: 'object',
            properties: {
              system_field_name: { type: 'string', description: '셀렉터 필드명 (예: BRD_CD, KWD_NM, END_DT, KWD_NM_CLASS, KWD_BRD_NM)' }
            },
        ...
        purpose: {
          type: 'string',
          description: '이 조회를 실행하는 이유'
        }
      },
      required: ['filters_product', 'start_dt', 'end_dt']
    }
  },
  // ── Google Trends 도구 ──
  {
...
이전 데이터는 대시보드의 날짜 탭에서 계속 확인 가능합니다.`,
  ...
  },
  {
    ...
    description: `KG API를 통해 두 기간의 검색량을 비교하여 급상승/급하락 키워드 트렌드 변화를 분석합니다.
현재 기간(current)과 이전 기간(previous)을 지정하여 키워드별 검색량 증감을 도출합니다.`,
    input_schema: {
      type: 'object',
      properties: {
        filters_product: {
          ...
          items: {
            ...
            properties: {
              system_code: { type: 'string', description: '필터 값 (예: 브랜드코드)' },
              system_field_name: { type: 'string', description: '필터 필드명 (예: BRD_CD)' }
            },
            required: ['system_code', 'system_field_name']
          },
          description: '상품 필터 배열 (BRD_CD 필수 포함)'
        ...
        current_start_dt: {
          type: 'string',
          description: '현재 기간 시작일 (YYYY-MM-DD)'
        },
        current_end_dt: {
          type: 'string',
          description: '현재 기간 종료일 (YYYY-MM-DD)'
        },
        previous_start_dt: {
          type: 'string',
          description: '이전 기간 시작일 (YYYY-MM-DD)'
        },
        previous_end_dt: {
          type: 'string',
          description: '이전 기간 종료일 (YYYY-MM-DD)'
        },
        keyword_class: {
          type: 'string',
          description: '키워드 분류 필터 (자사/경쟁사/일반/라이프스타일/모니터링키워드)'
        }
      },
      required: ['filters_product', 'current_start_dt', 'current_end_dt', 'previous_start_dt', 'previous_end_dt']
    }
  }
];
