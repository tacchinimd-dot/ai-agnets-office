...
- [x] ⏰ 스케줄 패널 UI: 스케줄 목록, 활성화 토글, 즉시 실행, 삭제, 새 스케줄 추가
- [x] 최근 리포트 이력 표시 (최대 50건 저장, 패널에 최근 5건)
- [x] 회의록 자동 연동: 자동 리포트 생성 시 회의록에 자동 기록
- [x] `server/schedules.json` 파일 기반 영속성 (서버 재시작 후에도 유지)
- [x] index.html 동기화

### Phase 5.1 — 대시보드/차트 시각화 패널 (완료)
- [x] `server/chart-builder.js` — 도구 결과를 Chart.js 호환 데이터로 자동 변환
- [x] 19개 도구별 최적 차트 타입 자동 결정 (bar, line, doughnut, radar, scatter)
- [x] 범용 Snowflake 결과 차트: 시계열(line) vs 카테고리(bar) 자동 판별
- [x] Chart.js 4.x CDN 통합 (외부 의존성 최소)
- [x] 📊 차트 패널 탭 추가 (최대 30개 차트 보관)
- [x] 에이전트별 차트 필터
- [x] 다크 테마 Chart.js 스타일링 (그리드, 툴팁, 범례)
- [x] 도구 실행 시 자동 차트 생성 + WebSocket 전송 (`chart_data` 메시지)
- [x] 스케줄러 자동 리포트에서도 차트 자동 생성
- [x] index.html 동기화

**차트 매핑:**
| 도구 | 차트 |
|------|------|
| 무신사 랭킹/요약 | 가격 바차트, 전환율 산점도, 카테고리 도넛, 브랜드 바차트 |
| TikTok 해시태그 | 조회수 바차트, 순위변동 도넛 |
| Google Trends | 검색어 바차트, 검색량 바차트 |
| 경쟁사 분석 | 가격 비교, 카테고리 도넛, Vision AI 레이더 |
| Snowflake 판매 | 채널별 라인/바차트 |
| 카테고리 성과 | 판매금액+재고 바차트 |
| 판매 TOP 스타일 | 판매금액 바차트 |
| 급상승 키워드 | 변화율 바차트 |
| 범용 SQL 결과 | 시계열 라인 or 카테고리 바차트 자동 판별 |

### Phase 5.2 — 에이전트 간 자동 트리거 (완료)
- [x] `server/triggers.js` — 조건부 연쇄 분석 트리거 엔진
- [x] 기본 트리거 5종:
  - 급상승 트렌드 → 경쟁사 확인 (김하늘 → 이서연)
  - 경쟁사 가격 변동 → 상품 기획 알림 (이서연 → 박도현)
  - 판매 실적 이상 → CEO 보고 (최재원 → 한준혁)
  - 트렌드 발견 → 상품 기획 제안 (김하늘 → 박도현)
  - 상품 기획 완료 → 데이터 검증 (박도현 → 최재원)
- [x] 5가지 조건 평가기: rising_detected, price_insight, anomaly_detected, new_trend, analysis_complete
- [x] 쿨다운 시스템 (중복 트리거 방지)
- [x] 도구 실행 후 자동 트리거 체크 (sendToolActivity 연동)
- [x] 트리거 발동 시 토스트 알림 + 회의록 자동 기록
- [x] ⏰ 스케줄 패널 내 트리거 관리 UI (토글, 삭제, 발동 이력)
- [x] `server/triggers.json` 영속성
- [x] index.html 동기화

### Phase 5.3 — 리포트 내보내기 (완료)
- [x] 대화 리포트 내보내기: MD(마크다운) / HTML 형식
- [x] 회의록 내보내기: MD(마크다운) / HTML 형식
- [x] 다크 테마 HTML 리포트 (독립 파일, 외부 의존성 없음)
- [x] 채팅 헤더에 MD/HTML 내보내기 버튼 추가
- [x] 회의록 필터 바에 📥 MD / 📥 HTML 내보내기 버튼 추가
...
| 2026-03-28 | **Phase 5.0 완료** — 에이전트 자율 분석 스케줄링 (5종 템플릿, 자동 실행, 리포트 저장, ⏰ 스케줄 패널) |
